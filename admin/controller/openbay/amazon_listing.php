...
class ControllerOpenbayAmazonListing extends Controller {
	public function create() {
		...
		if ($this->request->post) {
			...
			if ($result['status'] === 1) {
				...
				$this->response->redirect($this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&room_id=' . $this->request->post['room_id'] . $url, 'SSL'));
			}
		}
		...
		if (isset($this->request->get['room_id'])) {
			...
			$listing_status = $this->model_openbay_amazon->getRoomStatus($this->request->get['room_id']);

			if ($listing_status === 'processing' || $listing_status === 'ok') {
				$this->response->redirect($this->url->link('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&room_id=' . $this->request->get['room_id'] . $url, 'SSL'));
			} else if ($listing_status === 'error_advanced' || $listing_status === 'saved' || $listing_status === 'error_few') {
				$this->response->redirect($this->url->link('openbay/amazon_room', 'token=' . $this->session->data['token'] . '&room_id=' . $this->request->get['room_id'] . $url, 'SSL'));
			}
		} else {
			$this->response->redirect($this->url->link('extension/openbay/items', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$data['url_advanced']  = $this->url->link('openbay/amazon_room', 'token=' . $this->session->data['token'] . '&room_id=' . $this->request->get['room_id'] . $url, 'SSL');
		...
		if ($this->config->get('openbay_amazon_listing_tax_added')) {
			$data['price'] = $room_info['price'] * (1 + $this->config->get('openbay_amazon_listing_tax_added') / 100);
		} else {
		...
		if ($listing_status == 'error_quick') {
			$data['listing_errors'] = $this->model_openbay_amazon->getRoomErrors($room_info['room_id'], 3);
		}
	...
	public function edit() {
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&room_id=' . $room_id . $url, 'SSL'),
			'text' => $this->language->get('text_edit_heading'),
		);

		$status = $this->model_openbay_amazon->getRoomStatus($room_id);

		if ($status === false) {
			$this->response->redirect($this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&room_id=' . $room_id . $url, 'SSL'));
			return;
		}

		$data['room_links'] = $this->model_openbay_amazon->getRoomLinks($room_id);
		$data['url_return']  = $this->url->link('extension/openbay/items', 'token=' . $this->session->data['token'] . $url, 'SSL');
		if ($status == 'ok' || $status == 'linked') {
			$data['url_create_new']  = $this->url->link('openbay/amazon_listing/createNew', 'token=' . $this->session->data['token'] . '&room_id=' . $room_id . $url, 'SSL');
			$data['url_delete_links']  = $this->url->link('openbay/amazon_listing/deleteLinks', 'token=' . $this->session->data['token'] . '&room_id=' . $room_id . $url, 'SSL');
		}
		...
		$data['url_saved_listings']  = $this->url->link('openbay/amazon/savedListings', 'token=' . $this->session->data['token'] . '&room_id=' . $room_id, 'SSL');
	...
	public function createNew() {
		...
		$this->load->model('openbay/amazon');
		$this->model_openbay_amazon->deleteRoom($room_id);
		$this->response->redirect($this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&room_id=' . $room_id . $url, 'SSL'));
	}
	...
	public function deleteLinks() {
		...
		$links = $this->model_openbay_amazon->getRoomLinks($room_id);
		foreach ($links as $link) {
			$this->model_openbay_amazon->removeRoomLink($link['amazon_sku']);
		}
		$this->model_openbay_amazon->deleteRoom($room_id);
	...
	public function getRoomByAsin() {
		...
		$data = $this->model_openbay_amazon_listing->getRoomByAsin($this->request->post['asin'], $this->request->post['market']);
