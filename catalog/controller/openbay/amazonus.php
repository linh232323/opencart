...
class ControllerOpenbayAmazonus extends Controller {
	public function order() {
		...
		// If the order already exists on Genuine Tours, ignore it.
		if ($order_id) {
		...
		foreach ($order_xml->Items->Item as $item) {
			...
			$room_id = $this->model_openbay_amazonus_order->getRoomId((string)$item->Sku);
			$room_var = $this->model_openbay_amazonus_order->getRoomVar((string)$item->Sku);
			...
			$rooms[] = array(
				...
				'reward' => '0',
				'option' => $this->model_openbay_amazonus_order->getRoomOptionsByVar($room_var),
				'download' => array(),
			);
		...
		$this->model_openbay_amazonus_order->addAmazonusOrderRooms($order_id, $room_mapping);

		foreach($rooms as $room) {
			if($room['room_id'] != 0) {
				$this->model_openbay_amazonus_order->decreaseRoomQuantity($room['room_id'], $room['quantity'], $room['var']);
			}
		}
	...
	public function listing() {
		...
		$this->load->model('openbay/amazonus_room');
		...
		if ($data['status']) {
			$logger->write("Updating " . $data['room_id'] . ' as successful');
			$this->model_openbay_amazonus_listing->listingSuccessful($data['room_id']);
			$this->model_openbay_amazonus_room->linkRoom($data['sku'], $data['room_id']);
			$logger->write("Updated successfully");
		} else {
			$logger->write("Updating " . $data['room_id'] . ' as failed');
			$this->model_openbay_amazonus_listing->listingFailed($data['room_id'], $data['messages']);
			$logger->write("Updated successfully");
		}
	}
	...
	public function listingReport() {
		...
		if ($data) {
			$this->model_openbay_amazonus_room->addListingReport($data);
		}

		$this->model_openbay_amazonus_room->removeListingReportLock($request['marketplace']);
	...
	public function room() {
		...
		$this->load->model('openbay/amazonus_room');
		$this->load->library('log');
		$logger = new Log('amazonus_room.log');

		$logger->write("AmazonusRoom/inbound: incoming data");
		...
		if($status == "submit_error") {
			$message = 'Room was not submited to amazonus properly. Please try again or contact OpenBay.';
			$this->model_openbay_amazonus_room->setSubmitError($decoded_data['insertion_id'], $message);
		} else {
			$status = (array)$status;
			if($status['successful'] == 1) {
				$this->model_openbay_amazonus_room->setStatus($decoded_data['insertion_id'], 'ok');
				$insertion_room = $this->model_openbay_amazonus_room->getRoom($decoded_data['insertion_id']);
				$this->model_openbay_amazonus_room->linkRoom($insertion_room['sku'], $insertion_room['room_id'], $insertion_room['var']);
				$this->model_openbay_amazonus_room->deleteErrors($decoded_data['insertion_id']);

				$quantity_data = array(
					$insertion_room['sku'] => $this->model_openbay_amazonus_room->getRoomQuantity($insertion_room['room_id'], $insertion_room['var'])
				);
				...
				$msg = 'Room was not accepted by amazonus. Please try again or contact OpenBay.';
				$this->model_openbay_amazonus_room->setSubmitError($decoded_data['insertion_id'], $msg);
				...
				if(isset($decoded_data['error_details'])) {
					foreach($decoded_data['error_details'] as $error) {
						...
						$this->model_openbay_amazonus_room->insertError($error_data);
	...
	public function search() {
		...
		$this->model_openbay_amazonus_room->updateSearch($json);
	}
