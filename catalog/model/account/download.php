...
class ModelAccountDownload extends Model {
	...
	public function getTotalDownloads() {
		...
		if ($implode) {
			$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_room op ON (o.order_id = op.order_id) LEFT JOIN " . DB_PREFIX . "room_to_download p2d ON (op.room_id = p2d.room_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND (" . implode(" OR ", $implode) . ")");
