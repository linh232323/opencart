    ...
    <form id="room-form">
      ...
      <table class="table table-bordered table-hover">
        ...
        <thead id="table-head-2">
          <tr>
            <th class="text-right"><?php echo $text_autocomplete_room; ?></th>
        ...
        <tbody id="unlinked-items">
          <tr>
            <td class="text-right">
              <input type="hidden" id="new-room-id">
              <input id="new-room" type="text" class="form-control" autocomplete="off">
            </td>
            <td>
...
<script type="text/javascript"><!--
  ...
  function loadLinks() {
    $.ajax({
      ...
      type: 'get',
      dataType: 'json',
      data: 'room_id=' + encodeURIComponent($('#new-room-id').val()) + '&amazon_sku=' + encodeURIComponent($('#new-amazon-sku').val()),
      success: function (json) {
        var rows = '';
        for (i in json) {
          rows += '<tr>';
          rows += '<td class="text-left">' + json[i]['room_name'] + '</td>';
  ...
  $('#button-load').bind('click', function(e) {
    ...
    $.ajax({
      ...
      success: function (json) {
        ...
        var rows = '';
        for (i in json) {
          rows += '<tr id="room_row_' + json[i]['room_id'] + '_' + json[i]['var'] + '">';
            rows += '<td class="text-left">' + json[i]['room_name'] + '</td>';
            rows += '<td class="text-left">' + json[i]['model'] + '</td>';
            rows += '<td class="text-left">' + json[i]['combi'] + '</td>';
            rows += '<td class="text-left">' + json[i]['sku'] + '</td>';
            rows += '<td class="text-left">';
              rows += '<div class="amazon_sku_div_' + json[i]['room_id'] + '_' + json[i]['var'] + '">';
                rows += '<div class="row">';
                  rows += '<div class="col-sm-8 form-group">';
                    rows += '<input class="form-control amazon_sku_' + json[i]['room_id'] + '_' + json[i]['var'] + '"  type="text">';
                  rows += '</div>';
                  rows += '<div class="col-sm-4 form-group">';
                    rows += '<a class="btn btn-primary" onclick="addNewSkuField(' + json[i]['room_id'] + ', \'' + json[i]['var'] + '\')" data-toggle="tooltip" data-original-title="<?php echo $button_add; ?>"><i class="fa fa-plus-circle"></i></a>';
                  rows += '</div>';
                rows += '</div>';
              rows += '</div>';
            rows += '</td>';
            rows += '<td class="text-center"><a class="btn btn-primary" onclick="addNewLink(this, \'' + json[i]['room_id'] + '\', \'' + json[i]['var'] + '\')" data-toggle="tooltip" data-original-title="<?php echo $button_add; ?>"><i class="fa fa-plus-circle"></i></a></td>';
          rows += '</tr>';
        }
  ...
  function addLink(button, room_id, amazon_sku, variation) {
    $.ajax({
      url: '<?php echo html_entity_decode($add_item_link_ajax); ?>',
      type: 'get',
      dataType: 'json',
      async: 'false',
      data: 'room_id=' + encodeURIComponent(room_id) + '&amazon_sku=' + encodeURIComponent(amazon_sku) + '&var=' + encodeURIComponent(variation),
      beforeSend: function () {
  ...
  function addNewSkuField(room_id, variation) {
    var html = '';
    html += '<div class="amazon_sku_div_' + room_id + '_' + variation + '">';
      html += '<div class="row">';
        html += '<div class="col-sm-8 form-group">';
          html += '<input class="form-control amazon_sku_' + room_id + '_' + variation + '"  type="text">';
        html += '</div>';
        html += '<div class="col-sm-4 form-group">';
          html += '<a class="btn btn-danger remove_sku_icon_' + room_id + '_' + variation + '" onclick="removeSkuField(this, \'' + room_id + '\', \'' + variation + '\')"><i class="fa fa-minus-circle"></i></a>';
        html += '</div>';
      html += '</div>';
    html += '</div>';

    $(".amazon_sku_div_" + room_id + "_" + variation.replace(":", "\\:")).last().after(html);
  }

  function removeSkuField(icon, room_id, variation) {
    var removeIndex = $('.remove_sku_icon_' + room_id + '_' + variation.replace(':', '\\:')).index($(icon)) + 1;
    $(".amazon_sku_div_" + room_id + "_" + variation.replace(':', '\\:') + ":eq(" + removeIndex + ")").remove();
  }

  function addNewLink(button, room_id, variation) {
    var errors = 0;

    console.log(".amazon_sku_" + room_id + "_" + variation.replace(':', '\\:'));
    $(".amazon_sku_" + room_id + "_" + variation.replace(':', '\\:')).each(function (index) {
      if ($(this).val() == '') {
        errors++;
      }
    });
    ...
    $(".amazon_sku_" + room_id + "_" + variation.replace(':', '\\:')).each(function (index) {
      addLink(button, room_id, $(this).val(), variation);
    });

    $("#room_row_" + room_id + "_" + variation.replace(':', '\\:')).remove();
  }
  ...
  function addNewLinkAutocomplete() {
    ...
    if ($('#new-room-id').attr('label') != $('#new-room').val()) {
      alert('<?php echo $error_no_room_exists; ?>');
      return;
    }
    ...
    addLink('#add-new-button', room_id, amazon_sku, variation);

    $('#new-room').val('');
    $('#new-amazon-sku').val('');
    $('#new-room-id').val('');
    $('#new-room-id').attr('label', '');
    $('#openstock-option-selector').remove();
  }

  $('#new-room').autocomplete({
    ...
    'select': function (item) {
      $('#new-room-id').val(item['value']);
      $('#new-room').val(item['label']);
      $('#new-room-id').attr('label',item['label']);
      openstockCheck(item['value']);
    }
  });

  function openstockCheck(room_id) {
    $.ajax({
